...
/**
 * Dev: izinkan akses lewat IP LAN (mis. http://192.168.1.50:3000) agar chunk /_next/* tidak diblokir.
 * Tambahkan host lain: NEXT_ALLOWED_DEV_ORIGINS=192.168.1.50,192.168.1.51
 */
function parseAllowedDevOrigins() {
  const raw = process.env.NEXT_ALLOWED_DEV_ORIGINS || "192.168.1.50";
  return String(raw)
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean);
