...
func main() {
	...
	// Paritas JSON dengan root::landing_ping (PHP). Next memakai GET /api/config untuk health; endpoint ini untuk selaras kontrak PHP.
	mux.HandleFunc("GET /api/public/landing/ping", handler.PublicLandingPing)
	...
	mux.HandleFunc("GET /api/ppat/booking/{nobooking}/billing-preview", ppatHandler.GetBillingPreviewBadan)
	...
	// GET /api/config — frontend Next + pemetaan master_config.php (subset aman field "master")
	mux.HandleFunc("/api/config", func(w http.ResponseWriter, r *http.Request) {
		...
		out := map[string]interface{}{
			...
			"environment":   cfg.Env,
			"service":       "ebphtb-backend-go",
			"configSource":  "go_master_parity_env",
			"phpConfigPeer": "conf.cfg/master_config.php",
		}
		if cfg.Master != nil {
			out["master"] = cfg.Master.ToPublicMap()
		}
