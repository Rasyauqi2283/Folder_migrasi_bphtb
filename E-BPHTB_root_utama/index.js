...
// main part
// Sandbox: --sandbox atau SANDBOX=1 → load .env.sandbox, port 3600
if (process.argv.includes('--sandbox')) process.env.SANDBOX = '1';
// Startup ringkas (seperti Next.js): set STARTUP_QUIET=1 atau NODE_ENV=development
if (!process.env.STARTUP_QUIET && process.env.NODE_ENV === 'development') process.env.STARTUP_QUIET = '1';
...
import { buildRuntimeConfig } from './backend/bootstrap/runtime-config.js';
import { createLogger } from './backend/bootstrap/logger.js';
import { registerAppMiddleware } from './backend/bootstrap/middleware.js';
import { runServer } from './backend/bootstrap/start-server.js';
...
if (!startupQuiet) console.log('🌐 API_URL:', API_URL, `(sumber: ${apiUrlSource})`);
const apiUrlRef = { current: API_URL };

const logger = createLogger();
const { morganMiddleware } = registerAppMiddleware(app, {
  pool,
  runtime,
  apiUrlRef,
  logger
...
app.use('/design-n-script', express.static(path.join(__dirname, 'E-BPHTB_root_utama', 'public', 'design-n-script')));
app.use('/asset', express.static(path.join(__dirname, 'E-BPHTB_root_utama', 'public', 'asset')));
// Serve uploads with a fixed base path
app.use('/uploads', express.static(path.join(__dirname, 'E-BPHTB_root_utama', 'public', 'uploads')));
// Serve penting_F_simpan files - menggunakan path yang benar
app.use('/penting_F_simpan', express.static(path.join(__dirname, 'E-BPHTB_root_utama', 'public', 'penting_F_simpan')));
...
// Admin endpoints moved to adminRoutes.js
    ...
    try {
        ...
        const transformedData = result.rows.map(row => {
            ...
            return {
                ...
                pemverifikasi_nama: row.pemverifikasi_nama || row.pemverifikasi || 'N/A',
                _metadata: {
          ...
          if (emailDataQuery.rows.length > 0) {
            ...
            if (userEmail && noValidasiEmail) {
              try {
                ...
                const baseUrl = isLocalDev ? LOCAL_BASE : (process.env.BASE_URL || process.env.FRONTEND_URL || `http://localhost:${process.env.PORT || 3000}`);
...
app.post('/api/pv/send-to-lsb', async (req, res) => {
    const client = await pool.connect();
    const { nobooking, no_validasi, no_registrasi } = req.body;
    ...
    try {
      ...
      if (existing.rowCount > 0) {
        ...
        await client.query(
          `INSERT INTO lsb_1_serah_berkas
            (nobooking, userid, namawajibpajak, namapemilikobjekpajak,
             status, trackstatus, keterangan, no_registrasi)
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
          [
            nobooking,
            booking.userid,
