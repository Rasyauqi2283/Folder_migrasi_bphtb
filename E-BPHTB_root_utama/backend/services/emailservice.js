...
try {
  ...
  if (process.env.SENDGRID_API_KEY) {
    ...
    if (process.env.STARTUP_QUIET !== '1') console.log('✅ SendGrid email service initialized');
  }
  // SMTP fallback
    ...
    emailService = 'smtp';
    if (process.env.STARTUP_QUIET !== '1') console.log('✅ SMTP email service initialized (fallback)');
  } else if (isLocalDev) {
    emailService = 'noop';
    if (process.env.STARTUP_QUIET !== '1') console.log('📧 Email disabled for local dev (no credentials).');
  } else {
...
export async function sendEmailViaSendGrid(to, subject, text, html = null) {
  if (emailService === 'noop') {
    console.log('📧 Email skipped (local dev) to', to);
...
export async function sendEmailSafe(mailOptions) {
  try {
    if (!emailService || emailService === 'noop') {
      if (emailService === 'noop') {
        console.log('📧 Email skipped (local dev)', { to: mailOptions?.to, subject: mailOptions?.subject });
      } else {
                ...
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
                    ...
                    <div style="background: #f8f9fa; padding: 30px; border-radius: 0 0 10px 10px; border: 1px solid #e9ecef;">
                        ...
                        <div style="text-align: center; margin: 30px 0;">
                            <a href="${process.env.FRONTEND_URL || `http://localhost:${process.env.PORT || 3000}`}/login.html" 
