...
function toAbsolutePublicPath(p) {
    ...
    return path.resolve(process.cwd(), 'E-BPHTB_root_utama', 'public', normalized);
}

export default function registerGeneratePdfVerifParaf(app, pool) {
        ...
        try {
            ...
            const q = `
                SELECT DISTINCT
                    ...
                    avpv.special_parafv AS pv_special_parafv,
                    bk.verified_at AS bank_verified_at
            ...
            // Logo
            try {
                const logoAbs = path.resolve(process.cwd(), 'E-BPHTB_root_utama', 'public', 'asset', 'Logobappenda_pdf.png');
                const pageWidth = 595;
            ...
            try {
                ...
                if (penelitiSigAbs && fs.existsSync(penelitiSigAbs)) {
                    ...
                    const processedImage = await sharp(penelitiSigAbs).trim().toBuffer();
                    doc.image(processedImage, sigX, sigY, { width: sigWidth, height: sigHeight });
                }
            } catch (_) {}
            ...
            try {
                ...
                if (penelitiSigAbs && fs.existsSync(penelitiSigAbs)) {
                    const sigWidth = 80;
                    const sigHeight = 38;
                    const boundaryY = 683; // garis batas bawah kotak "Dengan huruf"
                    const leftMargin = 30;
            ...
            const verifiedAtDate = (() => {
                try {
                    if (!data.bank_verified_at) return null;
                    const d = new Date(data.bank_verified_at);
...
try {
    ...
    console.warn("Failed to render stamp:", err?.message || err);
            ...
            drawCenteredText(doc, ` ${pvName} `, col4X, signatureYPosition + 70, columnWidth);
...
// Export function untuk generate PDF Verif Paraf ke file (untuk email attachment)
export async function buildVerifParafPdf({ pool, nobooking, outputPath, sessionUser = null }) {
    console.log(`[PDF-VERIF-PARAF-FILE] Generating PDF to file for nobooking: ${nobooking}`);
    ...
    // Query data sama seperti endpoint
    const tracking = await pool.query(
        'SELECT userid, nama FROM pat_1_bookingsspd WHERE nobooking = $1',
        [nobooking]
    );
    if (tracking.rows.length === 0) {
        throw new Error('Data untuk nobooking ini tidak ditemukan');
    }
    ...
    // Cek apakah tabel ttd_paraf_kasie tersedia
    const tableCheck = await pool.query(`SELECT to_regclass('public.ttd_paraf_kasie') AS reg`);
    const hasKasieTable = tableCheck.rows?.[0]?.reg !== null;
    ...
    // Query utama data PDF (sama seperti endpoint)
    const q = `
        SELECT DISTINCT
            pb.*, bp.*, o.*,
            vb.nama, vb.special_field,
    ...
    if (r.rows.length === 0) {
        throw new Error('Data not found for nobooking: ' + nobooking);
    }
    
    const data = r.rows[0];
    data.special_parafv = sessionUser?.special_parafv || data.special_parafv;
    
    // Create file write stream instead of response
    const writeStream = fs.createWriteStream(outputPath);
    const doc = new PDFKitDocument({ margin: 30, size: 'A4' });
    doc.pipe(writeStream);
    
    // Reuse semua logika PDF generation dari endpoint (line 119-540)
    // Logo
    try {
        const logoAbs = path.resolve(process.cwd(), 'E-BPHTB_root_utama', 'public', 'asset', 'Logobappenda_pdf.png');
        const pageWidth = 595;
    ...
    // Header (sama seperti endpoint, lines 127-540)
    doc.font('Helvetica-Bold').fontSize(16).text('SURAT SETORAN PAJAK DAERAH', 200, 40, { width: 450 });
    doc.font('Helvetica-Bold').fontSize(16).text('BEA PEROLEHAN HAK ATAS TANAH DAN BANGUNAN', 130, 60, { width: 450 });
    doc.font('Helvetica-Bold').fontSize(16).text('(SSPD - BPHTB)', 260, 80, { width: 450 });
    doc.moveTo(0, 105).lineTo(700, 105).stroke();
    ...
    try {
        ...
        if (penelitiSigAbs && fs.existsSync(penelitiSigAbs)) {
            ...
            const boundaryY = 683;
            const leftMargin = 30;
            const columnWidth = sigWidth + 30;
            const gapBetween = 24;
            const col3X = leftMargin + columnWidth + gapBetween + columnWidth + gapBetween - 30;
            const sigX = col3X + (columnWidth - sigWidth) / 2;
            const sigY = boundaryY - sigHeight - 2;
            doc.image(penelitiSigAbs, sigX, sigY, { width: sigWidth, height: sigHeight });
        }
    } catch (_) {}
    ...
    try {
        ...
        if (stampAbs && fs.existsSync(stampAbs)) {
            ...
            const stampX = col3X - 46;
            const stampY = signatureYPosition + 21;
            const processedStamp = await sharp(stampAbs).toBuffer();
            doc.save();
            doc.rotate(20, { origin: [stampX + stampWidth / 2, stampY + stampWidth / 2] });
            doc.image(processedStamp, stampX, stampY, { width: stampWidth });
            doc.restore();
    ...
    try {
        ...
        if (bphtbAbs && fs.existsSync(bphtbAbs)) {
            const processedImage = await sharp(bphtbAbs).trim().toBuffer();
            const sigY = signatureYPosition + 22;
            doc.image(processedImage, boxX + (stampWidth - signatureWidth)/2, sigY, { width: signatureWidth });
        }
    } catch (err) {
        console.warn('Failed to render BPHTB signature:', err?.message || err);
    ...
    // End document and wait for write stream to finish
    doc.end();
    ...
    console.log(`✅ [PDF-VERIF-PARAF-FILE] PDF generated successfully to: ${outputPath}`);
}
