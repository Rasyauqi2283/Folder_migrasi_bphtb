...
try {
    ...
    if (ENCRYPTION_KEY_RAW.length === 64) {
        ...
        if (process.env.STARTUP_QUIET !== '1') console.log('🔒 [SECURE] Generating key from string using PBKDF2');
    ...
    if (process.env.STARTUP_QUIET !== '1') console.log('🔒 [SECURE] Encryption key initialized successfully');
} catch (error) {
...
if (!fs.existsSync(SECURE_STORAGE_PATH)) {
    ...
    if (process.env.STARTUP_QUIET !== '1') console.log('🔒 [SECURE] Secure storage directory created:', SECURE_STORAGE_PATH);
}
...
if (!fs.existsSync(LOGS_PATH)) {
    ...
    if (process.env.STARTUP_QUIET !== '1') console.log('🔒 [SECURE] Logs directory created:', LOGS_PATH);
}
