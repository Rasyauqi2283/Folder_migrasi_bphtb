...
router.post('/register', (req, res, next) => {
  ...
  const { nama, nik, telepon, email, password, gender, ktpUploadId, verse, nip, special_field, pejabat_umum, divisi } = req.body;
  // verse: 'WP' | 'Karyawan' | 'PU' — default WP untuk backward compatibility
  const verseNorm = (String(verse || 'WP').trim() || 'WP').toLowerCase();
  const verseValue = verseNorm === 'pu' ? 'PU' : verseNorm === 'karyawan' ? 'Karyawan' : 'WP';
  ...
  // Validasi per verse (3 jalur pendaftaran)
  if (verseValue === 'Karyawan') {
    if (!nip || !String(nip).trim()) {
      return res.status(400).json({ success: false, message: 'NIP wajib diisi untuk pendaftaran Karyawan.' });
    }
  }
  if (verseValue === 'PU') {
    if (!special_field || !String(special_field).trim()) {
      return res.status(400).json({ success: false, message: 'Nama PPAT/Gelar (special_field) wajib diisi untuk pendaftaran PPAT/PPATS.' });
    }
    if (!pejabat_umum || !String(pejabat_umum).trim()) {
      return res.status(400).json({ success: false, message: 'Pejabat Umum (PPAT/Notaris) wajib diisi untuk pendaftaran PPAT/PPATS.' });
    }
    const divisiPu = String(divisi || '').trim().toUpperCase();
    if (divisiPu !== 'PPAT' && divisiPu !== 'PPATS') {
      return res.status(400).json({ success: false, message: 'Pilih divisi PPAT atau PPATS untuk pendaftaran Pejabat Pembuat Akta.' });
    }
  }
  ...
  try {
    ...
    if (resultEmail.rows.length > 0) {
      ...
      verseValue, verseValue === 'Karyawan' ? (nip || null) : null, verseValue === 'PU' ? (special_field || null) : null, verseValue === 'PU' ? (pejabat_umum || null) : null, verseValue === 'PU' ? (String(divisi).trim().toUpperCase() || null) : null,
      email
      ];
      ...
      // Jika email tidak ada, insert data baru (termasuk verse & field jalur Karyawan/PU)
      const insertQuery = `
      INSERT INTO a_1_unverified_users (nama, nik, telepon, email, password, foto, otp, verifiedstatus, fotoprofil, gender, verse, nip, special_field, pejabat_umum, divisi, created_at)
...
router.post('/verify-otp', async (req, res) => {
  ...
  try {
    ...
    // 4. Tentukan verse (default WP untuk data lama tanpa verse)
    const verseOut = (user.verse && ['WP', 'Karyawan', 'PU'].includes(user.verse)) ? user.verse : 'WP';
    let useridOut = '';
    let divisiOut = '';
    let ppatKhususOut = '';
    ...
    try {
      ...
      if (verseOut === 'WP') {
        ...
        useridOut = await generateUserID(client, 'Wajib Pajak');
      } else if (verseOut === 'PU') {
        divisiOut = (user.divisi === 'PPATS') ? 'PPATS' : 'PPAT';
        useridOut = await generateUserID(client, divisiOut);
        ppatKhususOut = (await generatePPATNumber(client)).toString();
      }
      // Karyawan: userid dan divisi kosong, nanti diisi admin via /api/users/update
      ...
      const insertValues = [
        ...
        '', useridOut, divisiOut, 'offline', ppatKhususOut, user.gender || null,
        verseOut, user.nip || null, user.special_field || null, user.pejabat_umum || null
      ];
      ...
      console.log(`✅ Verifikasi berhasil untuk ${email} (verse: ${verseOut}, userid: ${useridOut || '(admin assign)'})`);
      return res.json({
        success: true,
        ...
        data: {
          ...
          userid: useridOut || undefined,
          divisi: divisiOut || undefined
        }
      });
...
import { extractKTPWithFallback } from '../../../../E-BPHTB_MIgration/migrated/node/utils/ktpOCRClient.js';
...
router.post('/real-ktp-verification', secureUploadKTP.single('fotoktp'), async (req, res) => {
  ...
  try {
    ...
    // Wrap OCR dalam Promise dengan timeout (Python service first, fallback Tesseract)
    const ocrPromise = extractKTPWithFallback(imagePath);
    ...
    res.json({
      ...
      data: {
        ...
        rtRw: ocrResult.rtRw || null,
        kelurahan: ocrResult.kelurahan || null,
        kecamatan: ocrResult.kecamatan || null,
