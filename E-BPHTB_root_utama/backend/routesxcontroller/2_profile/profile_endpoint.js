...
router.delete('/update-profile-paraf', async (req, res) => {
  ...
  try {
    ...
    if (currentPath) {
      try {
        const fullPath = path.join(process.cwd(), 'E-BPHTB_root_utama', 'public', currentPath);
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (oldPhotoPath && !oldPhotoPath.includes('default-foto-profile')) {
        const fullOldPath = path.join(process.cwd(), 'E-BPHTB_root_utama', 'public', oldPhotoPath);
