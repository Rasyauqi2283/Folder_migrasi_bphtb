...
function validateForm() {
    ...
    var verse = verseEl ? (verseEl.value || 'WP').trim() : 'WP';
    if (verse === 'Karyawan') {
        var nipVal = document.getElementById('nip') ? document.getElementById('nip').value.trim() : '';
        if (!nipVal) {
            var nipErr = document.getElementById('nipError');
            if (nipErr) { nipErr.textContent = 'NIP wajib diisi untuk pendaftaran Karyawan'; nipErr.style.display = 'block'; }
            isValid = false;
        }
    }
    if (verse === 'PU') {
        var divisiPu = document.getElementById('divisi_pu') ? document.getElementById('divisi_pu').value : '';
        var specialFieldVal = document.getElementById('special_field') ? document.getElementById('special_field').value.trim() : '';
        var pejabatUmumVal = document.getElementById('pejabat_umum') ? document.getElementById('pejabat_umum').value.trim() : '';
        if (!divisiPu || (divisiPu !== 'PPAT' && divisiPu !== 'PPATS')) {
            alert('Pilih jenis akun PPAT atau PPATS.');
            isValid = false;
        }
        if (!specialFieldVal) {
            var sfErr = document.getElementById('specialFieldError');
            if (sfErr) { sfErr.textContent = 'Nama PPAT/Gelar wajib diisi'; sfErr.style.display = 'block'; }
            isValid = false;
        }
        if (!pejabatUmumVal) {
            var puErr = document.getElementById('pejabatUmumError');
            if (puErr) { puErr.textContent = 'Pejabat Umum wajib diisi'; puErr.style.display = 'block'; }
            isValid = false;
        }
    }
...
document.querySelector("form").addEventListener("submit", async (event) => {
    ...
    var verseInput = document.getElementById('verse');
    var verseVal = verseInput ? (verseInput.value || 'WP').trim() : 'WP';
    formData.append('verse', verseVal);

    if (verseVal === 'Karyawan') {
      var nipEl = document.getElementById('nip');
      if (nipEl) formData.append('nip', nipEl.value || '');
    }
    if (verseVal === 'PU') {
      var divisiPu = document.getElementById('divisi_pu');
      var specialField = document.getElementById('special_field');
      var pejabatUmum = document.getElementById('pejabat_umum');
      if (divisiPu) formData.append('divisi', divisiPu.value || '');
      if (specialField) formData.append('special_field', specialField.value || '');
      if (pejabatUmum) formData.append('pejabat_umum', pejabatUmum.value || '');
    }
