...
router.get('/counter/:userid', async (req, res) => {
    try {
        ...
        const result = await db.query(query, [userid, date || new Date().toISOString().split('T')[0]]);
        ...
        if (result.rows.length === 0) {
            ...
            const newResult = await db.query(query, [userid, date || new Date().toISOString().split('T')[0]]);
            return res.json({
                success: true,
...
router.post('/counter/:userid/increment', async (req, res) => {
    try {
        ...
        const counterInfo = await db.query('SELECT * FROM get_peneliti_counter($1, $2)', [
            userid, 
...
router.post('/counter/reset', async (req, res) => {
    try {
        await db.query('SELECT reset_daily_counters()');
...
router.post('/counter/auto-reset', async (req, res) => {
    try {
        await db.query('SELECT auto_reset_daily_counters()');
